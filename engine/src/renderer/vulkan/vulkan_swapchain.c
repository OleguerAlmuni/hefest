...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        HFATAL("Failed to aquire swapchain image.");
        return FALSE;
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    VkExtent2D max = context->device.swapchain_support.capabilities.minImageExtent;
    ...
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)hallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        HFATAL("Failed to find a supported depth format.");
    }
