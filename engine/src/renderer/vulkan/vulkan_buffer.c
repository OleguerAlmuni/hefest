...
b8 vulkan_buffer_create(
    ...
    hzero_memory(out_buffer, sizeof(vulkan_buffer));
    ...
    VK_CHECK(vkCreateBuffer(context->device.logical_device, &buffer_create_info, context->allocator, &out_buffer->handle));
    ...
    if (out_buffer->memory_index == -1) {
        HERROR("Unable to create vulkan buffer because the required memory type index was not found.");
        return false;
    }
...
b8 vulkan_bufer_resize(
    ...
    VkBuffer new_buffer;
    VK_CHECK(vkCreateBuffer(context->device.logical_device, &buffer_create_info, context->allocator, &new_buffer));
    ...
    // Make sure anything potentially using the buffer is finished.
