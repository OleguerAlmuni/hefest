...
b8 vulkan_material_shader_create(vulkan_context* context, texture* default_diffuse, vulkan_material_shader* out_shader) {
    ...
    VkDescriptorType object_descriptor_types[VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT] = {
    ...
    VkDescriptorSetLayoutBinding object_bindings[VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT];
    hzero_memory(&object_bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    object_layout_info.bindingCount = VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT;
...
void vulkan_material_shader_update_object(vulkan_context* context, struct vulkan_material_shader* shader, geometry_render_data data) {
    ...
    hzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    accmulator += context->frame_delta_time;
    f32 s = (hfstsin(accmulator) + 1.0f) / 2.0f; // Scale from -1, 1 to 0, 1
