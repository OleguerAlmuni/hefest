...
void vulkan_image_view_create(
    ...
    view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D; // TODO: Make view type configurable.
    view_create_info.format = format;
    view_create_info.subresourceRange.aspectMask = aspect_flags;

    // TODO: Make mip level and array layer configurable.
...
void vulkan_image_transition_layout(
    ...
    // Don't care about the old_layout - transition to optimal layout (for the underlying implementation).
    if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        HFATAL("Unsupported layout transition.");
        return;
    }
...
void vulkan_image_copy_from_buffer(
    ...
    hzero_memory(&region, sizeof(VkBufferImageCopy));
