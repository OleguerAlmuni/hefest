...
void vulkan_renderpass_create(
    ...
    // If coming from a previous pass, the image should already be VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL. Otherwise undefined.
    color_attachment.initialLayout = has_prev_pass ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED;
    // If going on to another pass, stay in VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL. Otherwise transition for presentation.
    ...
    if (do_clear_depth) {
        ...
        hzero_memory(&attachment_descriptions[attachment_description_count], sizeof(VkAttachmentDescription));
    ...
    // Attachments used for multialiasing color attachments.
...
void vulkan_renderpass_begin(
    ...
    VkClearValue clear_values[RENDERPASS_ATTACHMENT_COUNT];
    hzero_memory(clear_values, sizeof(VkClearValue) * RENDERPASS_ATTACHMENT_COUNT);

    b8 do_clear_color = (renderpass->clear_flags & RENDERPASS_CLEAR_COLOR_BUFFER_FLAG) != 0;
    if (do_clear_color) {
        hcopy_memory(clear_values[begin_info.clearValueCount].color.float32, renderpass->clear_color.elements, sizeof(f32) * 4);
