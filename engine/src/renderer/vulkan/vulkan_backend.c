...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        HINFO("Searching for layer: %s...", required_validation_layer_names[i]);
        b8 found = false;
        ...
        if (!found) {
            HFATAL("Required validation layer is missing: %s", required_validation_layer_names[i]);
            return false;
        }
    }
    ...
    // World renderpass. Clears everything and hands the image on to the UI pass.
    ...
    // UI renderpass. Draws on top of the world pass, so it clears nothing, and
    // presents, so it is the last pass.
    vulkan_renderpass_create(
        &context,
    ...
    // World and swapchain framebuffers.
    regenerate_framebuffers();

    // Command buffers.
    create_command_buffers(backend); // will reserve; ensure it sets length (see below)

    // Sync objects.
    context.image_available_semaphores = darray_reserve(VkSemaphore, context.swapchain.max_frames_in_flight);
    darray_length_set(context.image_available_semaphores, context.swapchain.max_frames_in_flight);

    context.queue_complete_semaphores = darray_reserve(VkSemaphore, context.swapchain.image_count);
    darray_length_set(context.queue_complete_semaphores, context.swapchain.image_count);

    context.in_flight_fence_count = context.swapchain.max_frames_in_flight;

    for (u8 i = 0; i < context.swapchain.max_frames_in_flight; ++i) {
        VkSemaphoreCreateInfo semaphore_create_info = (VkSemaphoreCreateInfo){VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
        ...
        VkFenceCreateInfo fence_create_info = (VkFenceCreateInfo){VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
    ...
    // In-flight fences should not yet exist at this point, so clear the list. These are stored as
    // pointers because the initial state should be 0, and will be 0 when not in use. The actual
    // fences are not owned by this list.
    for (u32 i = 0; i < context.swapchain.image_count; ++i) {
    ...
    if (!vulkan_ui_shader_create(&context, &context.ui_shader)) {
        HERROR("Error loading built-in UI shader.");
        return false;
    }
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend) {
    ...
    // Destroy per-frame acquire semaphores & fences
    if (context.image_available_semaphores) {
        u32 acq_count = darray_length(context.image_available_semaphores);
        for (u32 i = 0; i < acq_count; ++i) {
            ...
            if (i < context.in_flight_fence_count) {
    ...
    if (context.queue_complete_semaphores) {
        u32 rf_count = darray_length(context.queue_complete_semaphores);
        for (u32 i = 0; i < rf_count; ++i) {
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    HASSERT(context.current_frame < context.swapchain.max_frames_in_flight);

    // Check if recreating swap chain and boot out.
    if (context.recreating_swapchain) {
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if (!vulkan_result_is_success(result)) {
            HERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if (context.framebuffer_size_generation != context.framebuffer_size_last_generation) {
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if (!vulkan_result_is_success(result)) {
            HERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    VkResult fence_result = vkWaitForFences(context.device.logical_device, 1, &context.in_flight_fences[context.current_frame], true, 0xffffffffffffffff);
    if (!vulkan_result_is_success(fence_result)) {
        HFATAL("In-flight fence wait failure! error: %s", vulkan_result_string(fence_result, true));
        return false;
    }
    ...
    // If this swapchain image is already in flight, wait its fence now (before reusing its command buffer).
    if (context.images_in_flight[context.image_index] != 0) {
        VkResult image_fence_result = vkWaitForFences(context.device.logical_device, 1, context.images_in_flight[context.image_index], true, 0xFFFFFFFFFFFFFFFFULL);
        if (!vulkan_result_is_success(image_fence_result)) {
            HFATAL("vkWaitForFences error: %s", vulkan_result_string(image_fence_result, true));
        }
    }
    ...
    // Reset the current frame fence so it will be signaled by the upcoming submission.
...
b8 vulkan_renderer_begin_renderpass(struct renderer_backend* backend, u8 renderpass_id) {
    ...
    switch (renderpass_id) {
        ...
        case BUILTIN_RENDERPASS_UI:
            ...
            break;
        default:
            HERROR("vulkan_renderer_begin_renderpass called on unrecognized renderpass id: %#02x", renderpass_id);
            return false;
    }
...
b8 vulkan_renderer_end_renderpass(struct renderer_backend* backend, u8 renderpass_id) {
    ...
    switch (renderpass_id) {
        ...
        case BUILTIN_RENDERPASS_UI:
            ...
            break;
        default:
            HERROR("vulkan_renderer_end_renderpass called on unrecognized renderpass id: %#02x", renderpass_id);
            return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    VkSubmitInfo submit_info = (VkSubmitInfo){VK_STRUCTURE_TYPE_SUBMIT_INFO};
    ...
    submit_info.pSignalSemaphores = &context.queue_complete_semaphores[context.image_index]; // per-image signal
    ...
    if (result != VK_SUCCESS) {
        HERROR("vkQueueSubmit failed with result: %s", vulkan_result_string(result, true));
        return false;
    }
    ...
    vulkan_swapchain_present(
        ...
        context.queue_complete_semaphores[context.image_index], // wait on the per-image semaphore
        context.image_index);
...
void create_command_buffers(renderer_backend* backend) {
    if (!context.graphics_command_buffers) {
        ...
        darray_length_set(context.graphics_command_buffers, context.swapchain.image_count);
        for (u32 i = 0; i < context.swapchain.image_count; ++i) {
            hzero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
        }
    }
    // Reallocate (ensure length already correct)
    for (u32 i = 0; i < context.swapchain.image_count; ++i) {
        if (context.graphics_command_buffers[i].handle) {
            vulkan_command_buffer_free(
                &context,
...
void regenerate_framebuffers() {
    ...
    for (u32 i = 0; i < image_count; ++i) {
        // World framebuffers: colour plus depth.
        ...
        // Swapchain framebuffers (UI pass). Output to the swapchain images, colour only.
...
b8 recreate_swapchain(renderer_backend* backend) {
    ...
    if (context.recreating_swapchain) {
        HDEBUG("recreate_swapchain called when already recreating. Booting.");
        return false;
    }

    // Detect if the window is too small to be drawn to
    if (context.framebuffer_width == 0 || context.framebuffer_height == 0) {
        HDEBUG("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return false;
    }
    ...
    // Regenerate world and swapchain framebuffers.
    regenerate_framebuffers();

    create_command_buffers(backend);

    // Destroy old per-image render-finished semaphores (if any)
    if (context.queue_complete_semaphores) {
        u32 old_count = darray_length(context.queue_complete_semaphores);
    ...
    for (u32 i = 0; i < context.swapchain.image_count; ++i) {
        ...
        VK_CHECK(vkCreateSemaphore(context.device.logical_device, &sci, context.allocator,
                                   &context.queue_complete_semaphores[i]));
    }

    // Clear the in-flight image tracking for the new image count.
    for (u32 i = 0; i < context.swapchain.image_count; ++i) {
        context.images_in_flight[i] = 0;
    }
...
void vulkan_renderer_create_texture(const u8* pixels, texture* texture) {
    ...
    texture->internal_data = (vulkan_texture_data*)hallocate(sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    ...
    vulkan_buffer staging;
    vulkan_buffer_create(&context, image_size, usage, memory_property_flags, true, &staging);

    vulkan_buffer_load_data(&context, &staging, 0, image_size, 0, pixels);

    // NOTE: Lots of assumptions here, different textures will require
    ...
    if (!vulkan_result_is_success(result)) {
        HERROR("Error creating texture sampler: %s", vulkan_result_string(result, true));
        return;
    }
...
void vulkan_renderer_destroy_texture(texture* texture) {
    ...
    if (data) {
        ...
        hzero_memory(&data->image, sizeof(vulkan_image));
        vkDestroySampler(context.device.logical_device, data->sampler, context.allocator);
        data->sampler = 0;

        hfree(texture->internal_data, sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    }
    hzero_memory(texture, sizeof(struct texture));
}
        ...
        switch (material->type) {
            case MATERIAL_TYPE_WORLD:
                if (!vulkan_material_shader_acquire_resources(&context, &context.material_shader, material)) {
                    HERROR("vulkan_renderer_create_material - Failed to acquire world shader resources.");
                    return false;
                }
                break;
            case MATERIAL_TYPE_UI:
                if (!vulkan_ui_shader_acquire_resources(&context, &context.ui_shader, material)) {
                    HERROR("vulkan_renderer_create_material - Failed to acquire UI shader resources.");
                    return false;
                }
                break;
            default:
                HERROR("vulkan_renderer_create_material - unknown material type.");
                return false;
        }

        HTRACE("Renderer: Material created.");
        return true;
    }

    HERROR("vulkan_renderer_create_material called with nullptr. Creation failed.");
    return false;
}
        ...
        if (material->internal_id != INVALID_ID) {
            switch (material->type) {
                ...
                case MATERIAL_TYPE_UI:
                    ...
                    break;
                default:
                    HERROR("vulkan_renderer_destroy_material - unknown material type.");
                    break;
            }
        } else {
            HWARN("vulkan_renderer_destroy_material called with internal_id=INVALID_ID. Nothing was done.");
        }
    } else {
        HWARN("vulkan_renderer_destroy_material called with nullptr. Nothing was done.");
    }
}
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    if (!vertex_count || !vertices) {
        HERROR("vulkan_renderer_create_geometry requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return false;
    }

    // Check if this is a re-upload. If it is, the old data needs freeing afterward.
    ...
    if (!internal_data) {
        HFATAL("vulkan_renderer_create_geometry failed to find a free index for a new geometry upload. Adjust config to allow for more.");
        return false;
    }
...
void vulkan_renderer_destroy_geometry(geometry* geometry) {
    if (geometry && geometry->internal_id != INVALID_ID) {
        ...
        // Clean up data.
        hzero_memory(internal_data, sizeof(vulkan_geometry_data));
...
void vulkan_renderer_draw_geometry(geometry_render_data data) {
    ...
    switch (m->type) {
        ...
        case MATERIAL_TYPE_UI:
            ...
            break;
        default:
            HERROR("vulkan_renderer_draw_geometry - unknown material type: %i", m->type);
            return;
    }
