...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    for (u32 i = 0; i < index_count; ++i) {
        ...
        // TODO: Enable this for a future enhancement (multi-threading).
    ...
    // TODO: This should be done through config
...
b8 select_physical_device(vulkan_context* context) {
    ...
    if (!context->device.physical_device) {
        HERROR("No physical device were found which meet the requirements.");
        return FALSE;
    }
...
b8 physical_device_meets_requirements(
    ...
    for (u32 i = 0; i < queue_family_count; ++i) {
        ...
        if (queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
            ...
            // the chances of it being a dedicated transfer queue.
    ...
    if (
        ...
        HTRACE("Graphics family index: %i", out_queue_info->graphics_family_index);
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            ...
            HINFO("Required swapchain support not present in device. Skipping.");
            return FALSE;
        }
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = hallocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        HINFO("Required extension not found in device: '%s'. Skipping.", requirements->device_extension_names[i]);
                        hfree(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                        return FALSE;
                    }
                }
            }
        ...
        if (requirements->sampler_anistropy && !features->samplerAnisotropy) {
            HINFO("Device does not support samplerAniostropy. Skipping.");
            return FALSE;
        }
