...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    // TODO: Should be using an allocator here.
    material_config* resource_data = hallocate(sizeof(material_config), MEMORY_TAG_MATERIAL_INSTANCE);
    // Set some defaults.
    ...
    string_ncopy(resource_data->name, name, MATERIAL_MAX_NAME_LENGTH);
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (strings_equal_insensitive(trimmed_var_name, "version")) {
            ...
            string_ncopy(resource_data->name, trimmed_value, MATERIAL_MAX_NAME_LENGTH);
        ...
        } else if (strings_equal_insensitive(trimmed_var_name, "type")) {
            // TODO: Other material types.
            if (strings_equal_insensitive(trimmed_value, "ui")) {
