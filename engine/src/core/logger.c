...
void log_output(log_level level, const char* message, ...) {
    ...
    // Technically imposes a 32k character limit on a sigle log entry, but...
    // Don't do that.
    char out_message[32000];
    hzero_memory(out_message, sizeof(out_message));
    
    char out_message2[32000];
    hzero_memory(out_message2, sizeof(out_message2));

    // Format original message.
    // NOTE: Oddly enough, MS's headers override the GCC/Clang va_list type with a "typedef char* va_list" in some
    // cases, and as a result throws a strange error here. The workaround for now is to just use __builtin_va_list,
    // which is the type GCC/Clang's va expects.
    ...
    string_format(out_message2, "%s%s\n", level_strings[level], out_message);
