...
void* hallocate(u64 size, memory_tag tag) {
    if (tag == MEMORY_TAG_UNKNOWN) {
        HWARN("hallocate called using MEMORY_TAG_UNKNOWN. Re-class this allocation.");
    }
...
void* hcopy_memory(void* destination, const void* source, u64 size) {
    return platform_copy_memory(destination, source, size);
}

void* hset_memory(void* destination, i32 value, u64 size) {
