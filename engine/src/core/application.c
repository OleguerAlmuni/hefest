...
b8 application_create(game* game_inst) {
    ...
    game_inst->application_state = hallocate(sizeof(application_state), MEMORY_TAG_APPLICATION);
    app_state = game_inst->application_state;
    app_state->game_inst = game_inst;
    app_state->is_running = false;
    app_state->is_suspended = false;

    u64 systems_allocator_total_size = 64 * 1024 * 1024; // 64 m
    ...
    // run.sh runs the binary from bin/, where post-build.sh has copied the asset tree.
    ...
    if (!resource_system_initialize(&app_state->resource_system_memory_requirement, app_state->resource_system_state, resource_sys_config)) {
        HFATAL("Failed to initialize resource system. Aborting application");
        return false;
    }
    ...
    if (!material_system_initialize(&app_state->material_system_memory_requirement, app_state->material_system_state, material_sys_config)) {
        HFATAL("Failed to initialize material system. Aborting application");
        return false;
    }
    ...
    if (!geometry_system_initialize(&app_state->geometry_system_memory_requirement, app_state->geometry_system_state, geometry_sys_config)) {
        HFATAL("Failed to initialize geometry system. Aborting application");
        return false;
    }
    ...
    uiverts[0].texture_coordinates.x = 0.0f;    //
    uiverts[0].texture_coordinates.y = 0.0f;    // 2    1

    uiverts[1].position.y = f;
    uiverts[1].position.x = f;
    uiverts[1].texture_coordinates.x = 1.0f;
    uiverts[1].texture_coordinates.y = 1.0f;

    uiverts[2].position.x = 0.0f;
    uiverts[2].position.y = f;
    uiverts[2].texture_coordinates.x = 0.0f;
    uiverts[2].texture_coordinates.y = 1.0f;

    uiverts[3].position.x = f;
    uiverts[3].position.y = 0.0f;
    uiverts[3].texture_coordinates.x = 1.0f;
    uiverts[3].texture_coordinates.y = 0.0f;
    ...
    if (!app_state->game_inst->initialize(app_state->game_inst)) {
        HFATAL("Game initialization failed.");
        return false;
    }
...
b8 application_run() {
    ...
    app_state->last_time = app_state->clock.elsapsed;
    f64 running_time = 0;
    u8 frame_count = 0;
    ...
    while(app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            clock_update(&app_state->clock);
            f64 current_time = app_state->clock.elsapsed;
...
b8 application_on_event(u16 code, void* sender, void* listener_inst, event_context context) {
    switch (code) {
        case EVENT_CODE_APPLICATION_QUIT: {
            HINFO("EVENT_CODE_APPLICATION_QUIT recieved, shutting down.\n");
            app_state->is_running = false;
            return true;
        }
    }
