...
b8 texture_system_initialize(u64* memory_requirement, void* state, texture_system_config config) {
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    hzero_memory(state_ptr->registered_textures, sizeof(texture) * count);
    for (u32 i = 0; i < count; ++i) {
        state_ptr->registered_textures[i].id = INVALID_ID;
        state_ptr->registered_textures[i].generation = INVALID_ID;
    }

    // Create defauly textures for use in the system.
...
texture* texture_system_acquire(const char* name, b8 auto_release) {
    ...
    if (strings_equal_insensitive(name, DEFAULT_TEXTURE_NAME)) {
        HWARN("texture_system_acquire called for default texture. Use texture_system_get_default_texure for texture 'default'");
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            if (!t || ref.handle == INVALID_ID) {
                HFATAL("texture_system_acquire - Texture system cannot hold anymore textures. Adjust configuration to allow more.");
                return 0;
            }
            ...
            t->id = ref.handle;
            HTRACE("Texture '%s' does not yet exist. Created, and ref_count is now %i.", name, ref.reference_count);
        } else {
            HTRACE("Texture '%s' already exists, ref count increased to %i.", name, ref.reference_count);
        }
    ...
    HERROR("texture_system_acquire failed to acquire texture '%s'. Null pointer will be returned.", name);
    return 0;
}
...
void texture_system_release(const char* name) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        if (ref.reference_count == 0) {
            HWARN("Tried to release non-existent texture: '%s'", name);
            return;
        }
        
        // Take a copy of the name since it will be wiped out by destroy
        // (as passed in name is generally a pointer to the actual textures name).
        ...
        if (ref.reference_count == 0 && ref.auto_release) {
            ...
            HTRACE("Released texture '%s', now has a reference count of '%i' (auto_release=%s).", name_copy, ref.reference_count, ref.auto_release ? "true" : "false");
        }
...
texture* texture_system_get_default_texture() {
    ...
    HERROR("texture_system_get_default_texture called before texture system initialization. Null pointer returned.");
    return 0;
}
...
b8 create_default_textures(texture_system_state* state) {
    ...
    state->default_texture.width = DEFAULT_TEXTURE_DIMENSION;
    state->default_texture.height = DEFAULT_TEXTURE_DIMENSION;
    state->default_texture.channel_count = DEFAULT_TEXTURE_CHANNELS;
...
b8 load_texture(const char* name, texture* t) {
    ...
    if (!resource_system_load(name, RESOURCE_TYPE_IMAGE, &img_resource)) {
        HERROR("Failed to load image resource for texture '%s'.", name);
        return false;
    }
