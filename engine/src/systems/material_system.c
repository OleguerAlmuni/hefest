...
material* material_system_acquire(const char* name) {
    ...
    if (!load_configuration_file(full_file_path, &config)) {
        HERROR("Failed to load material file: '%s'. Null pointer will be returned.", full_file_path);
        return 0;
    }
...
material* material_system_acquire_from_config(material_config config) {
    ...
    if (strings_equal_insensitive(config.name, DEFAULT_MATERIAL_NAME)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            for (u32 i = 0; i < count; ++i) {
                if (state_ptr->registered_materials[i].id == INVALID_ID) {
                    // A free slot has been found. Use its index at the handle.
                    ref.handle = i;
            ...
            m->id = ref.handle;
            HTRACE("Material '%s' does not yet exist. Created, and ref_count is now '%i'.", config.name, ref.reference_count);
    ...
    HERROR("material_system_acquire_from_config - Failed to acquire material '%s'. Null pointer will be returned.", config.name);
    return 0;
}

void material_system_release(const char *name) {
    // Ignore release requests for the default material.
    if (strings_equal_insensitive(name, DEFAULT_MATERIAL_NAME)) {
        return;
    }
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, name, &ref)) {
        if (ref.reference_count == 0) {
            HWARN("Tried to release non-existent material: '%s'", name);
            return;
        }
        ref.reference_count--;
        if (ref.reference_count == 0 && ref.auto_release) {
            ...
            HTRACE("Released material '%s'. Material unloaded because ref_count=0 and auto_release=true.", name);
        } else {
            HTRACE("Released material '%s', now has a reference count of '%i' (auto_release=%s).", name, ref.reference_count, ref.auto_release ? "true" : "false");
        }
...
material* material_system_get_default() {
    ...
    HFATAL("material_system_get_default called before system is initialized.");
    return 0;
}
...
b8 load_material(material_config config, material* m) {
    ...
    // name
    string_ncopy(m->name, config.name, MATERIAL_MAX_NAME_LENGTH);
    ...
    if (string_length(config.diffuse_map_name) > 0) {
        ...
        if (!m->diffuse_map.texture) {
            HWARN("Unable to load texture '%s' for material '%s', using default.", config.diffuse_map_name, m->name);
    ...
    if (!renderer_create_material(m)) {
        HERROR("Failed to acquire renderer resources to create material '%s'.", m->name);
        return false;
    }
...
b8 create_default_material(material_system_state* state) {
    ...
    string_ncopy(state->default_material.name, DEFAULT_MATERIAL_NAME, MATERIAL_MAX_NAME_LENGTH);
    ...
    if (!renderer_create_material(&state->default_material)) {
        HFATAL("Failed to acquire renderer resources for default material. Application cannot continue.");
        return false;
    }
...
b8 load_configuration_file(const char* path, material_config* out_config){
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        // Assume a max of 64 characters for the variable length.
        ...
        if (strings_equal_insensitive(trimmed_var_name, "version")) {
            // TODO: version
        } else if (strings_equal_insensitive(trimmed_var_name, "name")) {
            string_ncopy(out_config->name, trimmed_value, MATERIAL_MAX_NAME_LENGTH);
        } else if (strings_equal_insensitive(trimmed_var_name, "diffuse_map_name")) {
            string_ncopy(out_config->diffuse_map_name, trimmed_value, TEXTURE_NAME_MAX_LENGTH);
        } else if (strings_equal_insensitive(trimmed_var_name, "diffuse_color")) {
            // Parse the color.
            if (!string_to_vec4(trimmed_value, &out_config->diffuse_color)) {
                HWARN("Error parsing diffuse_color in file '%s'. Using default one instead.", path);
