...
typedef struct geometry_config {
    ...
    char material_name[MATERIAL_MAX_NAME_LENGTH];
} geometry_config;
 ...
 *
 * @return A pointer to the default 2D geometry.
 */
...
/**
 * @brief Generates a configuration for an axis-aligned cube. Faces are not
 * shared between vertices so that each one carries its own normal, which is
 * what lighting requires.
 */
