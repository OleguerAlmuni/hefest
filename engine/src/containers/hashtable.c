...
void hashtable_create(u64 element_size, u32 element_count, void* memory, b8 is_pointer_type, hashtable* out_hashtable) {
    ...
    hzero_memory(out_hashtable->memory, element_size * element_count);
}
...
b8 hashtable_set(hashtable* table, const char* name, void* value) {
    if (!table || !name || !value) {
        HERROR("hashtable_set requires table, name and value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        HERROR("hashtable_set should not be used with tables that have pointer types. Use hashtable_set_ptr instead.");
        return false;
    }
...
b8 hashtable_set_ptr(hashtable* table, const char* name, void** value) {
    if (!table || !name) {
        HWARN("hashtable_set_ptr requires table and name  to exist.");
        return false;
    }
    if (!table->is_pointer_type) {
        HERROR("hashtable_set_ptr should not be used with tables that do not have pointer types. Use hashtable_set instead.");
        return false;
    }
...
b8 hashtable_get(hashtable* table, const char* name, void* out_value) {
    if (!table || !name || !out_value) {
        HWARN("hashtable_get requires table, name and out_value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        HERROR("hashtable_get should not be used with tables that have pointer types. Use hashtable_set_ptr instead.");
        return false;
    }
    u64 hash = hash_name(name, table->element_count);
...
b8 hashtable_get_ptr(hashtable* table, const char* name, void** out_value) {
    if (!table || !name || !out_value) {
        HWARN("hashtable_get_ptr requires table, name and out_value to exist.");
        return false;
    }
    if (!table->is_pointer_type) {
        HERROR("hashtable_get_ptr should not be used with tables that do not have pointer types. Use hashtable_get instead.");
        return false;
    }
...
b8 hashtable_fill(hashtable* table, void* value) {
    if (!table || !value) {
        HWARN("hashtable_fill requires table and value to exist.");
        return false;
    }
    if (table->is_pointer_type) {
        HERROR("hashtable_fill should not be used with tables that have pointer types.");
        return false;
    }
