...
- **UI renderpass and UI shader.** Materials gain a `material_type` (`world` or `ui`), set from a `type=` key in the `.hmt` file, and the renderer switches on it to acquire, release and draw through the right shader. `geometry_config` becomes format-agnostic — `vertex_size`/`index_size` plus `void*` buffers — so the geometry system can feed both `vertex_3d` world geometry and `vertex_2d` UI geometry, and a default 2D geometry joins the default 3D one. `render_packet` carries a separate UI geometry list that the frontend draws in the UI pass. The four resource loaders' near-identical `unload` functions are factored into `loaders/loader_utils.c`.
